...
def scale_analysis():
    """
    Análise de escala de Bejan para a placa vertical isotérmica: mostra como o
    balanço de ordens de grandeza das equações de transporte prevê os dois
    regimes de Prandtl e a lei Nu ~ Ra^(1/4), validando contra a solução de
    similaridade (Ostrach) e a correlação de Churchill–Chu.
    """
    import numpy as np

    # Curva de Nu reduzido (Nu/Ra^(1/4)) vs Pr — só depende de Pr; calculada UMA vez.
    pr_grid = np.logspace(-2.0, 2.0, 22)
    nubar_sim = np.array([nc.reduced_nu_similarity(p) for p in pr_grid])
    nubar_corr = np.array([nc.reduced_nu_churchill(p) for p in pr_grid])
    ...
    Tinf = widgets.FloatSlider(value=300.0, min=250.0, max=600.0, step=5.0,
                               description="T_∞ [K]:", continuous_update=False)
    H    = widgets.FloatSlider(value=0.30, min=0.02, max=3.0, step=0.02,
                               description="H [m]:", continuous_update=False)
    ...
    def _update(*args):
        with out:
            ...
            p, Tf = _props()
            Ra = nc.rayleigh(Ts.value, Tinf.value, H.value, p)
            Gr = Ra / p.Pr
            bal = nc.scale_momentum_balance(p.Pr)
            est = nc.scale_estimates_vertical(Ra, p.Pr)
            ch = est["chosen"]
            regime_bl = nc.regime_vertical_plate(Ra)

            # Nu concretos neste Ra: similaridade exata e correlação laminar
            sim = nc.solve_similarity(p.Pr)
            Nu_sim = nc.nu_avg_similarity(Gr, sim["minus_theta0p"])
            Nu_corr = nc.nu_vertical_plate_laminar(Ra, p.Pr)

            print("=== Análise de escala (Bejan) — placa vertical ===")
            print(f"T_f = {Tf:.1f} K | Pr = {p.Pr:.3g} | H = {H.value:.3g} m | "
                  f"Ra = {Ra:.3e} ({regime_bl})")
            print(f"Regime de balanço: {bal['driver']}  "
                  f"(inércia/atrito = 1/Pr = {bal['inertia_over_friction']:.3g})")
            if bal["transition"]:
                print("  perto de Pr ~ 1 os três termos são comparáveis — regime "
                      "intermediário entre os dois limites de escala.")
            print(f"Escala:  δ_t/H ~ {ch['delta_t_over_H']:.2e}   |   {ch['law']}")
            print("-" * 52)
            print("Nu neste Ra (coeficiente O(1) vem do exato/correlação):")
            print(f"  similaridade (Ostrach)   Nu = {Nu_sim:8.2f}")
            print(f"  correlação (Churchill)   Nu = {Nu_corr:8.2f}"
                  f"   (dif. {abs(Nu_sim-Nu_corr)/Nu_corr*100:.1f}%)")
            print(f"  Nu reduzido Nu/Ra^(1/4)  = {Nu_corr/Ra**0.25:.3f}"
                  f"  (ver estrela no gráfico)")
            if regime_bl == "turbulento":
                print("Obs.: Ra > 1e9 — a lei de escala Nu~Ra^(1/4) é da camada "
                      "laminar; acima da transição vale Nu~Ra^(1/3).")

            viz.dashboard_scale_analysis(p.Pr, Ra, pr_grid, nubar_sim, nubar_corr)

    for w in [fluido, Ts, Tinf, H, nu_w, alpha_w, k_w, beta_w]:
        w.observe(_update, "value")

    # Dedução da análise de escala (mostrada uma vez, no topo)
    deducao = r"""
**Análise de escala (Bejan) — de onde vem Nu ~ Ra$^{1/4}$**

Equações da camada-limite ($x$ ao longo da placa, $y$ normal), escalas
$x\sim H$, $y\sim\delta_t$, $u\sim U$:

- **Energia** (convecção ~ condução na camada térmica): $\;U\dfrac{\Delta T}{H}\sim\alpha\dfrac{\Delta T}{\delta_t^{2}}\;\Rightarrow\; U\sim\dfrac{\alpha H}{\delta_t^{2}}$
- **Momento**: inércia $\dfrac{U^2}{H}$, atrito $\dfrac{\nu U}{\delta_t^{2}}$, empuxo $g\beta\Delta T$. A razão $\dfrac{\text{inércia}}{\text{atrito}}=\dfrac{\alpha}{\nu}=\dfrac{1}{Pr}$.

O empuxo (que sempre move o fluido) equilibra o **maior** dos dois:

| Regime | Balança | $\delta_t/H$ | Nu |
|---|---|---|---|
| $Pr\gg1$ | atrito–empuxo | $Ra^{-1/4}$ | $\sim Ra^{1/4}$ |
| $Pr\ll1$ | inércia–empuxo | $(Ra\,Pr)^{-1/4}$ | $\sim (Ra\,Pr)^{1/4}$ |

A análise fornece o **expoente**; o coeficiente $O(1)$ vem da similaridade/correlação
— é o que o gráfico da direita confirma.
"""
    display(Markdown(deducao))

    _update()
    display(widgets.VBox([
        widgets.HBox([fluido]),
        widgets.HBox([Ts, Tinf, H]),
        widgets.HBox([nu_w, alpha_w, k_w, beta_w]),
    ]), out)
